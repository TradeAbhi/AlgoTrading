...
import { usWeeklyApi } from '../api';

function UsWeeklyBreakout() {
  ...
  const loadData = async () => {
    try {
      const [allState, watchingData, alertedData, high52Data] = await Promise.all([
        usWeeklyApi.getState(),
        usWeeklyApi.getWatching(),
        usWeeklyApi.getAlerted(),
        usWeeklyApi.get52WeekHighs()
      ]);
      setState(allState);
      setWatching(watchingData);
      setAlerted(alertedData);
      setHigh52Week(high52Data);
    } catch (error) {
  ...
  const handleUpload = async () => {
    if (!tickers.trim()) {
      setMessage('Please enter tickers');
      return;
    }
    ...
    try {
      ...
      const result = await usWeeklyApi.uploadTickers(tickerList);
      setMessage(`Uploaded and seeded ${result.tickersLoaded} tickers`);
      setTickers('');
      await loadData();
    } catch (error) {
      setMessage('Error uploading tickers: ' + error.message);
    }
    setLoading(false);
  };
  ...
  const handleCapture = async () => {
    setLoading(true);
    try {
      const result = await usWeeklyApi.capture();
      setMessage(`Seeded ${result.tickersLoaded} tickers`);
      await loadData();
    } catch (error) {
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.title}>🇺🇸 US Weekly Breakout Scanner</h2>
      ...
      {/* Ticker Upload Section */}
      <div style={styles.section}>
        <h3 style={styles.sectionTitle}>Upload Tickers</h3>
        <textarea
          value={tickers}
        ...
        <button 
          ...
          style={styles.button}
        >
          {loading ? 'Processing...' : 'Upload & Seed'}
        </button>
      </div>
      ...
      <div style={styles.tabs}>
        {['all', 'watching', 'alerted', '52wk'].map(tab => (
          <button
            key={tab}
          ...
          >
            {tab.toUpperCase()} ({tab === 'all' ? state.length : 
              tab === 'watching' ? watching.length :
              tab === 'alerted' ? alerted.length : high52Week.length})
          </button>
        ))}
      </div>
        ...
        <table style={styles.table}>
          <thead>
            <tr>
              ...
              {activeTab === 'all' && <th style={styles.th}>52W High</th>}
            </tr>
          </thead>
          <tbody>
            {getDisplayData().map((item, index) => (
              <tr key={index} style={styles.tr}>
                ...
                {activeTab === 'all' && (
                  <td style={styles.td}>
                    {item.is52WeekHigh ? <span style={styles.high52}>✓</span> : '—'}
                  </td>
                )}
              </tr>
            ))}
...
export default UsWeeklyBreakout;
