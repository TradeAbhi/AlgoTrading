...
public class MarketSentimentService {
    ...
    /**
     * Sends a standalone A/D ratio alert to Telegram.
     * Called every 5 min (9:15–9:45) and every 15 min (9:45–15:30).
     *
     * Message format:
     *   📊 A/D Ratio | 09:30
     *   Advances:  312  Declines: 188
     *   A/D Ratio: 1.66 (62.4% advancing)
    ...
    public void sendAdvanceDeclineAlert() {
        try {
            ...
            String signal;
            String emoji;
            if      (adRatio >= 2.0)  { signal = "Very Bullish";    emoji = "🟢🟢"; }
            else if (adRatio >= 1.5)  { signal = "Bullish Breadth"; emoji = "🟢";   }
            else if (adRatio >= 0.85) { signal = "Neutral Breadth"; emoji = "🟡";   }
            else if (adRatio >= 0.5)  { signal = "Bearish Breadth"; emoji = "🔴";   }
            else                      { signal = "Very Bearish";    emoji = "🔴🔴"; }
            ...
            sb.append("━━━━━━━━━━━━━━━━━━━━\n");
            sb.append("Ratio  : *").append(fmt2(adRatio)).append("*")
              .append("  (").append(fmt1(advPct)).append("% advancing)\n");
            sb.append("Signal : ").append(signal).append(" ").append(emoji).append("\n");
 
            telegramService.sendMessage(sb.toString());
            log.info("A/D alert sent — advances={} declines={} ratio={} signal={}",
                    advances, declines, fmt2(adRatio), signal);
