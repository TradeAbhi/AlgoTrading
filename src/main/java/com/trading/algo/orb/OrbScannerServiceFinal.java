...
public class OrbScannerServiceFinal {
    ...
    /** Previous day level filter: free space must be at least 1% of current price. */
        ...
        for (Map.Entry<String, String> entry : keyToSymbol.entrySet()) {
            ...
            try {
                ...
                if (rangeWidth < MIN_RANGE_PCT) {
                    log.debug("[ORB] {} skipped — opening range too narrow ({:.2f}% < {:.1f}%)",
                        symbol, String.format("%.2f", rangeWidth), String.format("%.1f", MIN_RANGE_PCT));
                    continue;
                }

                // Fetch previous day's close, high, and low
                double prevClose = fetchPrevDayClose(symbol, instrumentKey);
                double prevHigh = fetchPrevDayHigh(symbol, instrumentKey);
                double prevLow = fetchPrevDayLow(symbol, instrumentKey);
                ...
                log.debug("[ORB] Seeded {} H:{} L:{} range:{}% | PrevDay H:{} L:{} C:{}",
    ...
    public void scanBreakouts() {
        ...
        List<String> skippedBuy = new ArrayList<>();
        List<String> skippedSell = new ArrayList<>();
                ...
                if (!state.isBuyAlerted()) {
                    if (close > state.getRollingHigh()) {
                        ...
                        if (candle.getVolume() < (long)(state.getOpeningCandleVolume() * MIN_VOLUME_MULTIPLIER)) {
                            ...
                            skippedBuy.add(String.format("%s (low volume)", symbol));
                        } else {
                            ...
                            if (close > state.getPrevDayHigh()) {
                                // Already above prev day high — sufficient room, trigger BUY
                                ...
                                double bufferRoom = (state.getPrevDayHigh() - close) / close * 100.0;
                                if (bufferRoom >= PREV_DAY_LEVEL_BUFFER_PCT) {
                                    // Sufficient buffer to reach previous day high
                                    state.setRollingLow(state.getPrevCandleLow());
                                    sendBuyAlert(state, candle);
                                    state.setBuyAlerted(true);
                                    buyFired++;
                                    log.info("[ORB] {} BUY alerted — {} buffer room to prev day high (₹{})",
                                        symbol, String.format("%.2f%%", bufferRoom),
                                        String.format("%.2f", state.getPrevDayHigh()));
                                } else {
                                    // Insufficient buffer — skip this alert
                                    log.info("[ORB] {} BUY SKIPPED — insufficient buffer ({} < {}) from prev day high (current:₹{}, prevHigh:₹{})",
                                        symbol, String.format("%.2f%%", bufferRoom), String.format("%.1f%%", PREV_DAY_LEVEL_BUFFER_PCT),
                                        String.format("%.2f", close), String.format("%.2f", state.getPrevDayHigh()));
                                    skippedBuy.add(String.format("%s (buffer: %.2f%% < %.1f%% to prev high ₹%.2f)",
                                        symbol, bufferRoom, PREV_DAY_LEVEL_BUFFER_PCT, state.getPrevDayHigh()));
                                }
                            }
                        }
                    ...
                    if (close < state.getRollingLow()) {
                        ...
                        if (candle.getVolume() < (long)(state.getOpeningCandleVolume() * MIN_VOLUME_MULTIPLIER)) {
                            log.debug("[ORB] {} SELL skipped — low volume (breakout:{} < {}x opening:{})",
                                symbol, candle.getVolume(), MIN_VOLUME_MULTIPLIER,
                                state.getOpeningCandleVolume());
                            skippedSell.add(String.format("%s (low volume)", symbol));
                        } else {
                            ...
                            if (close < state.getPrevDayLow()) {
                                // Already below prev day low — sufficient room, trigger SELL
                                ...
                                double bufferRoom = (close - state.getPrevDayLow()) / close * 100.0;
                                if (bufferRoom >= PREV_DAY_LEVEL_BUFFER_PCT) {
                                    // Sufficient buffer to reach previous day low
                                    state.setRollingHigh(state.getPrevCandleHigh());
                                    sendSellAlert(state, candle);
                                    state.setSellAlerted(true);
                                    sellFired++;
                                    log.info("[ORB] {} SELL alerted — {} buffer room to prev day low (₹{})",
                                        symbol, String.format("%.2f%%", bufferRoom),
                                        String.format("%.2f", state.getPrevDayLow()));
                                } else {
                                    // Insufficient buffer — skip this alert
                                    log.info("[ORB] {} SELL SKIPPED — insufficient buffer ({} < {}) from prev day low (current:₹{}, prevLow:₹{})",
                                        symbol, String.format("%.2f%%", bufferRoom), String.format("%.1f%%", PREV_DAY_LEVEL_BUFFER_PCT),
                                        String.format("%.2f", close), String.format("%.2f", state.getPrevDayLow()));
                                    skippedSell.add(String.format("%s (buffer: %.2f%% < %.1f%% to prev low ₹%.2f)",
                                        symbol, bufferRoom, PREV_DAY_LEVEL_BUFFER_PCT, state.getPrevDayLow()));
                                }
                            }
                        }
        ...
        // Log skipped stocks for analysis
        if (!skippedBuy.isEmpty()) {
            log.info("[ORB] Skipped BUY signals ({}): {}",
                skippedBuy.size(), String.join(", ", skippedBuy));
            telegramService.sendMessage(String.format(
                "⏭️ *ORB Alert Analysis*%n" +
                "*Skipped BUY Signals (%d):*%n%s%n%n📊 Recommendation: Review these stocks to validate the filter thresholds.",
                skippedBuy.size(), String.join("\n", skippedBuy)));
        }

        if (!skippedSell.isEmpty()) {
            log.info("[ORB] Skipped SELL signals ({}): {}",
                skippedSell.size(), String.join(", ", skippedSell));
            telegramService.sendMessage(String.format(
                "⏭️ *ORB Alert Analysis*%n" +
                "*Skipped SELL Signals (%d):*%n%s%n%n📊 Recommendation: Review these stocks to validate the filter thresholds.",
                skippedSell.size(), String.join("\n", skippedSell)));
        }

        log.info("[ORB] Scan done → BUY:{} SELL:{} | Skipped BUY:{} SELL:{}",
            buyFired, sellFired, skippedBuy.size(), skippedSell.size());
    }
    ...
    // ── Previous day close/high/low fetch ────────────────────────────────────
    /**
     * Fetches the previous trading day's data using the v3 historical endpoint.
     * Uses a 5-day window to guarantee we get at least one completed daily candle
    ...
    @SuppressWarnings("unchecked")
    private double fetchPrevDayHigh(String symbol, String instrumentKey) {
        try {
            String today    = java.time.LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            String fromDate = java.time.LocalDate.now().minusDays(5).format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            String encodedKey = instrumentKey.replace("|", "%7C");
            ...
            log.warn("[ORB] fetchPrevDayHigh failed for {}: {}", symbol, e.getMessage());
            return 0;
        }
    }

    @SuppressWarnings("unchecked")
    private double fetchPrevDayLow(String symbol, String instrumentKey) {
        try {
            String today    = java.time.LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            String fromDate = java.time.LocalDate.now().minusDays(5).format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            String encodedKey = instrumentKey.replace("|", "%7C");
            ...
            log.warn("[ORB] fetchPrevDayLow failed for {}: {}", symbol, e.getMessage());
            return 0;
        }
    }
