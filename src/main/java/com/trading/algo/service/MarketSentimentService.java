...
public class MarketSentimentService {
	...
	public void init() {
		...
		httpClient = HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(10))
				.followRedirects(HttpClient.Redirect.NORMAL).cookieHandler(cookieManager).build();

		try {
			getRaw(NSE_BASE, "/api/allIndices", null);
		} catch (Exception e) {
			log.error("[NSE] Prime failed: {}", e.getMessage());
		}
	}

	// -------------------------------------------------------------------------
	// DATA FETCHERS (FIXED PART)
	// -------------------------------------------------------------------------

	public Map<String, Object> fetchAdvanceDeclineData() throws Exception {
		...
		try {
			...
			Map<String, String> keyMap = upstoxInstrumentMasterService.resolveToInstrumentKeyMap(fnoSymbols);

			List<String> keys = new ArrayList<>(keyMap.values());

			List<com.trading.algo.dtos.WatchlistItem> quotes = upstoxMarketDataService.fetchLiveQuotes(keys);

			for (com.trading.algo.dtos.WatchlistItem q : quotes) {
				double change = q.getChangePercent();
				if (change > 0)
					advances++;
				else if (change < 0)
					declines++;
			...
			log.info("[AD] advances={} declines={} unchanged={}", advances, declines, unchanged);

		} catch (Exception e) {
			log.error("[AD] Failed: {}", e.getMessage());
			throw e;
		}
	...
	private byte[] getRaw(String base, String path, String token) throws Exception {
		HttpRequest.Builder b = HttpRequest.newBuilder().uri(URI.create(base + path)).timeout(Duration.ofSeconds(15))
	...
	public void sendAdvanceDeclineAlert() {
		try {
			...
			if (adRatio >= 2.0) {
				...
				signal = "Bullish Breadth";
				emoji = "🟢";
			} else if (adRatio >= 0.85) {
				signal = "Neutral Breadth";
				emoji = "🟡";
			} else if (adRatio >= 0.5) {
				signal = "Bearish Breadth";
				emoji = "🔴";
			} else {
				signal = "Very Bearish";
				emoji = "🔴🔴";
			...
			sb.append("📊 *A/D Ratio* | ").append(timeNow()).append("\n");
			sb.append("━━━━━━━━━━━━━━━━━━━━\n");
			...
			sb.append("━━━━━━━━━━━━━━━━━━━━\n");
			sb.append("Ratio  : *").append(fmt2(adRatio)).append("*").append("  (").append(fmt1(advPct))
					.append("% advancing)\n");
			...
			log.error("A/D alert failed: {}", e.getMessage());
		}
	}
